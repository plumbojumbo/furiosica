...
$(document).ready(function(){
	...
	var nick_timeout;
	//$("#form_register #nickname").attr("disabled", "disabled");
	$("#form_register ul.form_data li:first").after('<label for="nickname">Nickname</label><input name="nickname" type="text" id="nickname" disabled="disabled"/><p class="form_guide">This will be your nickname.</p></li>');
	$("#form_register #fullname").bind("keyup", function() {
		clearTimeout(nick_timeout);
		nick_timeout = setTimeout('GetNickname()', 500);
	});
...
function GetNickname() {
	var nick = $("#form_register #fullname").val();
	//if (nick.length > 0) {
		$.get("../../lib/ajax_nick.php",
			  { nick: $("#form_register #fullname").val() },
			  function(data, status) {
				$("#form_register #nickname").attr("value", data);
			  },
			  "text");
	//}
}
