...
class NudgeCommand extends Command
{
	...
	function execute($channel)
    {
		...
		if (!$other->email || !$other->emailnotifynudge) {
			$channel->error($this->user, _('This user doesn\'t allow nudges or hasn\'t confirmed or set his email yet.'));
            return;
        }
		if ($other->id == $user->id) {
			
        }
		mail_notify_nudge($this->user, $other);
		$channel->output($this->user, sprintf(_('You have nudged %s.'), $this->other));
	}
}
