...
class Notice extends Memcached_DataObject
{
    ...
    static function saveNew($profile_id, $content, $source=null, $is_local=1, $reply_to=null, $uri=null, $force_public=0) {
        ...
        $notice->uri = $uri;
		$notice->force_public = $force_public;
	...
	/**
	*	Checks notice text for force public tags ('#public') and removes them
	*	@return true if present, false otherwise
	*/
	static function checkPublicTag(&$content) {
		$content = preg_replace('/(?<=^|\s)#public(?:\s)|(?:^|\s)#public(?:$)/i', '', $content, -1, $count);
		if ($count) {
			return true;
		} else {
			return false;
    ...
    function publicStream($offset=0, $limit=20, $since_id=0, $before_id=0, $since=null)
    {
		...
		if (common_config('site', 'private') && !common_logged_in()) {
            $parts[] = 'force_public = 1';
        }
    ...
    function saveGroups()
    {
        ...
        foreach (array_unique($match[1]) as $nickname) {
			...
			// furiosica change: allow group posts from anybodyy
    ...
    function saveReplies()
    {
        ...
        for ($i=0; $i<count($names); $i++) {
			...
			// Uncomment this -> @user automatically replies to last notice of user if not specified otherwise
			/*
