...
class User extends Memcached_DataObject
{
    ...
    static function register($fields) {
        ...
        if ($code) {
			...
			# can use code only once
			if ($invite && ($invite->registered_user_id > 0)) {
				common_user_error(_('Invalid invitation code.'));
				return;
			}
		...
		// auto subscribe
		foreach(common_config('site', 'autosubscribe') as $otherid) {
			if (is_int($otherid)) {
				$subscription = new Subscription();
				$subscription->subscriber = $user->id;
				$subscription->subscribed = $otherid;
				$subscription->created = $user->created;
				$subscription->insert();
			}
		}
		...
		//Notice::saveNew($user->id, sprintf(_('has just joined %s!'), common_config('site', 'name')), 'web');
