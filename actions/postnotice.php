...
class PostnoticeAction extends Action
{
    ...
    function save_notice(&$req, &$consumer, &$token)
    {
		...
		$force_public = Notice::checkPublicTag($content);		
        $notice = Notice::staticGet('uri', $notice_uri);
        if (!$notice) {
            $notice = Notice::saveNew($remote_profile->id, $content, 'omb', false, 0, $notice_uri, ($force_public) ? 1 : 0);
            if (is_string($notice)) {
                common_server_serror($notice, 500);
                return false;
            }
			$offsite = (common_config('site', 'private')) ? ($force_public == 1) : true;
            common_broadcast_notice($notice, true, $offsite);
        }
        return true;
    }
}
