...
class InviteAction extends Action
{
    ...
    function handle($args)
    {
        ...
        if (!common_logged_in()) {
            ...
            return;
        } elseif (!in_array($user->id, common_config('site', 'admins'))) {
            $this->clientError(sprintf(_('Only admins can invite.')));
            return;
    ...
    function sendInvitation($email, $user, $personal)
    {
		...
		// purge old invites
		$existing = Invitation::staticGet('address', $email);
        ...
        $body = sprintf(_("%1\$s has invited you to join them on %2\$s (%3\$s).\n\n".
                          "%2\$s is a micro-blogging service that lets you keep up-to-date with what is happening at FURIOS.\n\n".
